...
class LaneOffset: 
  ...
  AUTO_MIN_SHOULDER_WIDTH_FACTOR = 0.25 # [unitless] shoulder width must be as this this factor of lane width
  ...
  def update_lane_info(self, md, v_ego, lane_width):
    ...
    if len(md.laneLines) == 4 and len(md.laneLines[0].t) == TRAJECTORY_SIZE \
      ...
      for i in range(2):
        self._road_edge_probs[i] = interp(md.roadEdgeStds[i], [.3, 1.], [1.0, 0.0])
      ...
      for i in range(3): 
        ...
        if ((i == 0 and l_prob > self.AUTO_MIN_ADJACENT_LANELINE_PROB) or l_prob > self.AUTO_MIN_LANELINE_PROB) \
          ...
          rll_y, lll_y = md.laneLines[i+1].y, md.laneLines[i].y
          ...
          # Reduce reliance on uncertain lanelines (less stringent for )
  ...
  def update(self, lane_pos=0., lane_width=LANE_WIDTH_DEFAULT, auto_active=False, md=None, sm=None): # 0., 1., -1. = center, left, right
    ...
    offset = self.OFFSET * self.offset_scale * self.lane_pos * lane_width
